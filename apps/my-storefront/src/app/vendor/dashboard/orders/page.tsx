...
import { getVendorOrders } from "@/lib/data/vendor"

interface VendorDashboardItem {
  id: string
  title: string
  quantity: number
}

interface VendorDashboardOrder {
  id: string
  display_id: number
  total_vendor_amount: number
  fulfillment_status: string
  payment_routing_status: string
  items?: VendorDashboardItem[]
}
...
export default async function VendorOrdersPage() {
  ...
  const orders: VendorDashboardOrder[] = await getVendorOrders()
      ...
      <div className="bg-white border border-neutral-200 rounded-xl shadow-sm overflow-hidden">
        ...
        {orders.length === 0 ? (
          <div className="p-16 text-center">
            <p className="text-neutral-400 text-sm font-medium">No active orders found requiring allocation.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-neutral-100 text-neutral-700">
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-neutral-50/40 transition-colors">
                    <td className="p-4 font-mono text-xs font-bold text-neutral-900">#{order.display_id}</td>
                    <td className="p-4">
                      <div className="flex flex-col gap-1.5">
                        {order.items?.map((item) => (
                          <div
                            key={item.id}
                            className="inline-flex items-center gap-1.5 text-xs text-neutral-700 bg-neutral-50 border border-neutral-200/60 px-2 py-1 rounded-md w-fit"
                          >
                    ...
                    <td className="p-4 font-semibold text-neutral-950">{formatCurrency(order.total_vendor_amount)}</td>
                    <td className="p-4">
                      <span
